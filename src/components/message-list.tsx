...
export const TIME_THRESHOLD = 5; // 閾値
...
export const MessageList = ({
  ...
  return (
    <div className="flex-1 flex flex-col-reverse pb-4 overflow-y-auto messages-scrollbar">
      ...
      <div
        className="h-1"
        // elは、DOMに要素が追加されたときに呼び出されます。
        ref={(el) => {
          if (el) {
            const observer = new IntersectionObserver(
              // IntersectionObserverEntry のオブジェクト配列の最初の要素のみを取り出す
              ([entry]) => {
              ...
              {
                threshold: 1.0, // 要素が100%表示されたときにコールバックを発火させる
              },
            );

            observer.observe(el); // targetとなる要素を引数に指定
